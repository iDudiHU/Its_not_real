...
namespace Invector.vCharacterController
{
	...
	public class vGenericInputDrawer : PropertyDrawer
	{
		...
		public static float heightOpen => ((EditorGUIUtility.singleLineHeight * 1.5f) * 3) + 10;
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			if (property.isExpanded)
			{
				...
				totalRect.height = EditorGUIUtility.singleLineHeight * 1.5f;

				totalRect.width -= 10;
				totalRect.x += 5;
				DrawInput(ref totalRect, keyboard, keyboardAxis, keyboardAxisInvert, isUnityInput, true);
				DrawInput(ref totalRect, joystick, joystickAxis, joystickAxisInvert, isUnityInput, true);
				DrawInput(ref totalRect, mobile, mobileAxis, mobileAxisInvert, isUnityInput, true);

			}
			GUI.enabled = true;

		}
		void DrawInput(ref Rect totalRect, SerializedProperty input, SerializedProperty axis, SerializedProperty invert, SerializedProperty isUnityInput = null, bool withKeys = false)
		{
			totalRect.y += EditorGUIUtility.singleLineHeight + 5f;
